...
def book_detail(book_name):
    ...
    temp_df = books[books['Book-Title'] == book_name].drop_duplicates('Book-Title')
    
    if temp_df.empty:
        return render_template('Book_recommend.html', error_message=f"Book '{book_name}' not found")
    ...
    if book_name in pt.index:
        ...
        for i in similar_items:
            item = []
            temp_rec = books[books['Book-Title'] == pt.index[i[0]]]
            item.extend(list(temp_rec.drop_duplicates('Book-Title')['Book-Title'].values))
            item.extend(list(temp_rec.drop_duplicates('Book-Title')['Book-Author'].values))
            item.extend(list(temp_rec.drop_duplicates('Book-Title')['Image-URL-M'].values))
            recommendations.append(item)
    
    # Get ISBN for reading the book online
    isbn = book_info.get('ISBN', '')
    google_books_url = f"https://books.google.com/books?isbn={isbn}" if isbn else ""
    
    return render_template('book_detail.html',
                        book_title=book_info['Book-Title'],
                        book_author=book_info['Book-Author'],
                        book_image=book_info['Image-URL-M'],
                        isbn=isbn,
